...
> OpenAI 官方命令行 AI 编程工具，支持自主执行代码任务，通过自定义 Provider 接入 MaxAPI AI。
...
<Steps>
  ...
  <Step title="创建配置文件">
    ...
    model = "gpt-4o"
    model_provider = "maxapi"       # 关键：指定默认 Provider
...
<Warning>
  **三个关键配置，缺一不可：**
  ...
  - `model_provider = "maxapi"` — 如果不设置，Codex 会把 `gpt-4o` 等已知模型名直接路由到 OpenAI 官方
